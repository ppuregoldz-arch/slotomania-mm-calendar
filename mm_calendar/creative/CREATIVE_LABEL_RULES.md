...
- Leave Status Creative empty for every non-Reuse brief until Creative starts. Monetization owns **Status MM** (`color_mkwes65f`): **`Waiting for MM`** when the agent finished writing the brief (MM reviews → **`Ready for Brief`** for Creative); **`MM work in progress`** for an incomplete New promo skeleton; **`Ready - no action needed`** for the Reuse summary. Reuse sets Status Creative to `done`.
