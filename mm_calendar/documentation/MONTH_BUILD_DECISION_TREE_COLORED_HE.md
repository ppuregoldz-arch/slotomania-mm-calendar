...
| מונח | מה זה בפועל |
|------|-------------|
| **Daily Deal (DD)** | ההצעה היומית המרכזית — «מה קונים היום» בחנות (**מוצר נפרד** ב-Monday) |
| **ADS (ADS PO)** | פרס ב**צפייה במודעה** — **מוצר נפרד** מ-DD, RYD, Core וכו'. **תמיד מתכננים בסוף היום**, אחרי שכל השאר מוכן |
...
flowchart TB
  ...
  subgraph LOOP["🟢 כל יום — רכישה ומשחק קודם"]
    D1["Daily Deal<br/>מוצר נפרד"]
    D2{"איזו הצעה שנייה?"}
  ...
  subgraph ADSLANE["🟢 ADS — בסוף היום בלבד"]
    AD1["שורת ADS PO<br/>מוצר ADS — לא DD"]
    AD2["פרס נמוך: מטבעות / ג'מים /<br/>reg נמוך · SKU עונה"]
  end

  subgraph PRIZE["🟣 פרסים — DD · הצעה 2 · Core · Decoy…<br/>לא ADS"]
    C2{"בבנק השבוע?"}
  ...
  C2 -->|לא| STOPC
  C2 -->|כן| C3 --> C4 --> OKP
  OKP --> D2 --> D3 --> D4 --> D5 --> AD1 --> AD2 --> D7
  D7 -->|כן| D8 --> D7
  D7 -->|לא| NEXT["יום הבא"]
  NEXT --> D1
  NEXT -->|אחרי יום אחרון| V1
  V1 --> V2 --> V3
  V3 -->|כן| STOP2
  V3 -->|לא| O1 --> O2
  O2 -->|כן| O3 --> O4
  ...
  style ADSLANE fill:#C8E6C9,stroke:#2E7D32,color:#1B5E20
...
**סדר קשיח:** קודם **כל מה שקשור לרכישה ומשחק**. **ADS רק אחרי** — מוצר נפרד, לא «המשך» של Daily Deal.

### שלב א — רכישה, מגבירים, משחק (לפני ADS)

| # | חובה? | מה |
|---|--------|-----|
| 1 | **כן** | **Daily Deal** (שורה נפרדת ב-Monday) |
| 2 | **כן** | **לפחות הצעה שנייה אחת** (או Popup / MFL שממלאים את התפקיד) |
| 3 | לפי שבוע / יום | **מגבירים** (MGAP, Gemback, x2 GGS…) — בתוך תקרות |
| 4 | **כן** (כמעט תמיד) | **משחק / ניקוז** — Core, MES, Spin Zone, Shiny Show (~3 בשבוע), Clan לפי template |
| 5 | לפי template | עוגנים שנכנסים ליום (Lotto, Piggy…) |

### שלב ב — **ADS בלבד, בסוף**

| # | מתי | מה |
|---|-----|-----|
| 1 | **רק אחרי** ששלב א סגור | שורה ב-**Product = ADS** — לא Daily Deal, לא Offers |
| 2 | תמיד אחרון | פרס **נמוך**: מטבעות, ג'מים, reg נמוך — **לא** Wild, Gold, Shiny חזק, Ace גבוה |
| 3 | עונה | אם יש קלף ב-ADS — **SKU של Short Term החי** (Blast / Battlesheep / SNL) + Mid Term בשורה |

**לא עושים:** לא «מחברים» ADS ל-DD באותה שורה; לא בוחרים פרס ADS בזמן שמתכננים DD; לא מדלגים על DD כי «ADS ממילא יש».

### 🟣 שלוש שאלות לפרסים — **DD, הצעה 2, Core, Decoy…** (לא ל-ADS)

| # | שאלה | אם לא |
|---|------|-------|
| **1** | הקלף **בבנק השבוע**? | 🔴 שואלים כלכלה |
| **2** | **משחק** או **רכישה**? | משחק: **אין Gold** · רכישה: Gold מותר; Wild **≤1** למקור הצעה |
| **3** | **יום יוקרה** (sale, Decoy d3, Counter PO, אירוע)? | שומרים Wild / Shiny Limited / 5★ לslots יוקרה; אחרת reg/ace/gold לפי בנק + בустר עונה |

### 🟢 ADS — כללים נפרדים (אחרי שאר היום)

| כן | לא |
|----|-----|
| Coins, Gems, 2★ Reg, פרסים «קטנים» | Wild, Gold, Shiny Limited, Ace גבוה |
| מוזכר Short Term + Mid Term **בתיאור** ADS | אותן שאלות «יום יוקרה» כמו Decoy d3 |

**SKU לפי Short Term חי (לרכישה + משחק, לא ל-ADS premium):**  
...
### תמחור באותו יום (DD + הצעה שנייה בלבד)

אם גם DD וגם ההצעה השנייה **עם** תמחור (High/Mid/Max) — **חייבים רמות שונות**.  
**ADS אין לו את אותו כלל תמחור מול DD** — זה מוצר אחר.

### אם היום **צפוף מדי**

**מורידים מלמטה למעלה** (לא נוגעים ב-DD וב-VFM):

1. **ADS** (אם חייבים לחתוך — ADS הוא הראשון לרדuction, כי הוא **אמור להיות אחרון** בכל מקרה)  
...
| בדיקה | מה מחפשים |
|--------|-----------|
| **כל יום** | DD + הצעה שנייה + Core (או חריג) — **ואז** שורת ADS **נפרדת** |
...
6. Clan לפי יום  
7. **לבסוף:** ADS (שורה נפרדת — לא חלק מ-DD)
...
**קודם קלט → שלד ועוגנים → לכל יום: DD, הצעה 2, מגבירים, משחק — ורק בסוף ADS (מוצר נפרד) → בדיקה → לוח. כותרת Monday מנצחת את התיאור.**
