...
| 3 | **כן** | משהו לניקוז מטבעות (Core / MES / …) — **בשני:** Dash Pass לפעמים מספיק |
| 4 | לפי שבוע | MGAP אם «נשאר מכסה» לשבוע (מקס **2**) |
| 5 | לפי template | Clan-Dash (לא נספר VFM) |
| 6 | **כן — אחרון** | **ADS PO** — מוצר **נפרד**, רק אחרי ששאר היום מוכן |

**אם היום צפוף מדי:** מוחקים **מלמטה למעלה**: **ADS** (ראשון לחתוך) → Lotto/Piggy → Core שני → מגבירים → **לא** נוגעים ב-DD וב-VFM.
...
## ג.3 — איך בוחרים **קלף / פרס**

**ADS** — ראה **ג.4** (מוצר נפרד, בסוף היום). כאן: DD, הצעה 2, Core, Decoy וכו'.

| # | שאלה | תשובה → פעולה |
|---|------|----------------|
| **א** | הקלף **מופיע בבנק השבוע** ב-guidelines? | **לא** → עוצרים / שואלים · **כן** → ממשיכים |
| **ב** | זה **משחק** (Core/MES) או **רכישה** (DD/RYD/Decoy/…)? | משחק → **אין Gold** · רכישה → Gold מותר, Wild **לכל היותר 1** למקור הצעה |
| **ג** | יום **יוקרה** (sale / BTS / Decoy d3 / Counter PO)? | **כן** → Wild / Shiny Limited / 5★ · **לא** → reg/ace/gold לפי בנק + **SKU של העונה החיה** |

## ג.4 — **ADS PO** (בסוף היום בלבד)

| כלל | פירוש |
|-----|--------|
| **סדר** | רק **אחרי** DD, VFM, מגבירים ו-Core/Clan |
| **מוצר** | שורת **ADS** — לא Daily Deal |
| **פרס** | Coins / Gems / reg נמוך — **לא** Wild, Gold, Shiny Limited, Ace גבוה |
| **עונה** | בשורת ADS מציינים Short Term + Mid Term החיים (לתיאור) |
